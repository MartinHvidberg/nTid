...
class Winlog:
    ...
    def hour_interval(self,timStart,timEnd):
        reverse = False
        if timStart > timEnd:
            timStart, timEnd = timEnd, timStart
            reverse = True
        
        delta = (timEnd.hour - timStart.hour) + (timEnd.minute - timStart.minute)/60.0 + (timEnd.second - timStart.second)/(60.0*60.0)
        if reverse:
            delta = 24 - delta
        return delta
    ...
    def report(self,strUser,strMode,strTime,dicStyle={"Report":"Default","DateFormat":"ISO","TimeFormat":"ISO","Reverse":False}):
        ...
        if doloR["DoLo_Status"] == 1:
            return "\n report() : DoLo for this user is LOE - Use showdolo() if you wan't to see the LOE..."
        ...
        if dicStyle["Report"] == "Classic+":           
            ...
            for dayN in self.listdates(strTime):
                ...
                if dayN in dicLOD:
                    ...
                    durN = self.hour_interval(timStart,timStop)                    
                    strReport += " : "+str(timStart)+" >> "+str(timStop) + " = (" + str(round(durN,1)) + " h)"
                else:
                    strReport += " :    No records"
            #for keyN in dicLOD:
            #    strReport += "\n k "+str(type(keyN))+str(keyN)
